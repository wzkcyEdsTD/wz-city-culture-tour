...
export default class JumpBubble {
  constructor(canvasNode, config = {}, callback) {
    if (!canvasNode || !canvasNode.getContext) {
      console.warn(
        "jumpBuffle，启用失败，canvas传参错误 或 浏览器不支持canvas"
      );
      this.error = true;
      return false;
    }
  ...
  create(imgsrc, before, after) {
    ...
    t.createImg(imgsrc).then(imgNode => {
      const imgInfo = {
        ...
        height:
          (imgwidth && imgNode.height * (imgwidth / imgNode.width)) ||
          imgNode.height,
      };
...
class DrawImg {
  ...
  setImgWidth() {
    ...
    if (imgWidth < originWidth) {
      //差值diffWidth的初始小气泡，逐渐变大为指定气泡的大小
      this.imgWidth += 1;
      this.imgHeight += 1;
    }
  }
  ...
  countYpxArr(effect, speedNum = 1) {
    //根据effect不同，设置气泡在y轴上位移距离，数组包含3个值，分别表示在y轴的3个阶段的位移距离
    const basePx = 2;
